...
**Free port 8501 (if 'Port 8501 is not available')**
...
Get-NetTCPConnection -LocalPort 8501 | Select-Object LocalAddress,LocalPort,RemoteAddress,State,OwningProcess
```
...
**Logs & Diagnostics**
- Streamlit server output appears in the terminal you used to run it; note the Local URL (http://localhost:8501).
...
- "READ RUNBOOK AND START STREAMLIT (CONDA)" — create the conda env (if missing) and start Streamlit on port 8501.
...
- "READ RUNBOOK AND FREE PORT 8501" — detect and stop processes holding port 8501 then start Streamlit (if requested).
...
- Confirm port 8501 is free. Start Streamlit and open http://localhost:8501 to verify UI.
...
- Quick way to free port 8501 (if required):
